...
from timm import create_model  # PyTorch Image Models (timm) for Vision Transformer

# MoCoV3 model with Swin Transformer
class MoCoV3Swin(nn.Module):
    def __init__(self, base_encoder='swin_base_patch4_window7_224', dim=256, mlp_dim=4096, T=0.2, momentum=0.999):
        super(MoCoV3Swin, self).__init__()

        self.T = T
        self.momentum = momentum

        # Create the encoders (Swin Transformer + projection MLP)
        self.encoder_q = create_model(base_encoder, pretrained=False, num_classes=0)  # no final classification head
        self.encoder_k = create_model(base_encoder, pretrained=False, num_classes=0)

        # Get the number of features from the Swin Transformer
        in_features = self.encoder_q.num_features

        # MLP projection head for query and key encoders
        self.encoder_q.head = nn.Sequential(
            nn.Linear(in_features, mlp_dim),
            nn.GELU(),
            nn.Linear(mlp_dim, dim)  # Projection into lower-dimensional space
        )
...
def contrastive_loss(q, k):
    # Reshape q and k to be (batch_size, num_features), flattening the spatial dimensions
    q = q.view(q.size(0), -1)  # (batch_size, num_features)
    k = k.view(k.size(0), -1)  # (batch_size, num_features)
    ...
    neg_logits = torch.einsum('nc,mc->nm', [q, k])
...
# Training loop for MoCoV3 with Swin Transformer
def train_moco_v3_swin(model, data_loader, optimizer, device, epochs=10):
    model.train()
    ...
    torch.save(model.state_dict(), '/home/noshin/JunkBox/fingerprint/Fingerprint_DS/Fingerprint_DS/moco_v3_swin_pretrained.pth')
    print("Model saved")
...
def augmentation(image_batch):
    aug_transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Swin input size is 224x224
...
# Data loader for both SOCOFing and FVC2000 datasets
...
if __name__ == "__main__":
    ...
    # Initialize Swin Transformer-based MoCoV3 model and optimizer
    model = MoCoV3Swin(base_encoder='swin_base_patch4_window7_224').to(device)
    optimizer = optim.SGD(model.parameters(), lr=0.03, momentum=0.9, weight_decay=1e-4)

    # Set up data loader
    socofing_dir = '/home/noshin/JunkBox/fingerprint/Fingerprint_DS/Fingerprint_DS/SOCOFing'  # Set the correct path to your SOCOFing dataset
    fvc_dir = '/home/noshin/JunkBox/fingerprint/Fingerprint_DS/Fingerprint_DS/merged_FVC'
    data_loader = get_data_loader(socofing_dir, fvc_dir)

    # Train the model
    train_moco_v3_swin(model, data_loader, optimizer, device, epochs=1)
